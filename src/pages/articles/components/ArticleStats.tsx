...
import { cardHover, containerVariants, fadeInUp } from '@/constants/animations';
import {
  BookOutlined,
  EditOutlined,
  EyeOutlined,
...
const ArticleStats: React.FC<ArticleStatsProps> = ({ statistics, loading }) => {
  ...
  return (
    ...
    >
            ...
            <div className="mt-4 flex items-center">
              <span
                className={`text-xs font-medium ${
                  card.trend.type === 'up' ? 'text-green-600' : 'text-red-600'
                }`}
              >
                {card.trend.type === 'up' ? '+' : '-'}
                {card.trend.value}%
              </span>
              <span className="text-xs text-gray-500 ml-1">vs 上个月</span>
            </div>
          )}
        </motion.div>
      ))}
      ...
      {/* 热门文章排行 */}
      {statistics?.top_viewed_articles &&
        statistics.top_viewed_articles.length > 0 && (
          <motion.div
            variants={fadeInUp}
            {...cardHover}
            className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:col-span-2 lg:col-span-2"
          >
      ...
      {/* 最受欢迎文章 */}
      {statistics?.top_liked_articles &&
        statistics.top_liked_articles.length > 0 && (
          <motion.div
            variants={fadeInUp}
            {...cardHover}
            className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:col-span-2 lg:col-span-2"
