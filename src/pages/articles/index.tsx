...
const ArticleManagementPage: React.FC = () => {
    ...
    const fetchArticles = async (params?: Partial<ArticleListQuery>) => {
        ...
        try {
            const query: ArticleListQuery = {
                ...
                access_type: filters.access_type === 'all' ? undefined : filters.access_type as any,
    ...
    const handleAccessChange = async (id: number, access_type: 'public' | 'private' | 'password', password?: string) => {
        try {
    ...
    const handleBatchStatusChange = async (status: 'draft' | 'published' | 'archived') => {
        if (selectedArticles.length === 0) {
        ...
        try {
            await Promise.all(selectedArticles.map(id => UpdateArticleStatus(id, { status })));
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    ...
                    <motion.div variants={itemVariants} className="mb-8">
                        ...
                        <p className="mt-2 text-gray-600">管理您的所有文章，包括发布、编辑、删除等操作</p>
                    </motion.div>
